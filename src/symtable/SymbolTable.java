...
public class SymbolTable {
	...
	// Pronalazi sve poÄ�etne taÄ�ke sekvenci u programu 
	// (prva naredba + sve naredbe na koje moÅ¾e da se doÄ‘e sa neke druge naredbe osim prethodne)
	public ArrayList<Integer> findSeqStarts() {
		ArrayList<Integer> seqStarts = new ArrayList<>();
		for (int i=0; i<scopes.size(); i++) {
			Scope scope = scopes.get(i);
			
			// Ako je tekuÄ‡i opseg petlja (a nije do-while)
			...
			// Ako je tekuÄ‡i opseg case (a nije prvi case u okviru switch-a)
		...
		// Ako veÄ‡ nismo dodali prvu naredbu, dodajemo je sad
		if (!seqStarts.contains(1))
			seqStarts.add(1);
		
		// Sortiramo sve poÄ�etke sekvenci (zbog preglednijeg ispisa)
	...
	public ArrayList<Jump> findJumps(ArrayList<IfElseStmt> ifElseStmts, ArrayList<BreakStmt> breakStmts) {
		...
		for (int i=0; i<scopes.size(); i++) {
			...
			// dodajemo skok (poÄ�etak petlje -> naredba posle petlje)
			// i skok (kralj petlje -> poÄ�etak petlje) 
			if (scope.getType() == ScopeType.LOOP && !scope.isDoLoop()) {
			...
			// dodajemo skok (kraj petlje -> poÄ�etak petlje)
			if (scope.getType() == ScopeType.LOOP && scope.isDoLoop()) 
				jumps.add(new Jump(scope.getRowEnd(), scope.getRowStart(), false, 2));
			
			// Ako je opseg if
			// dodajemo skok (poÄ�etak opsega -> naredba posle opsega)
			...
			// dodajemo skok (poÄ�etak case-a -> naredba posle case-a)
		...
		for (int i=0; i<breakStmts.size(); i++) {
			...
			// dodajemo skok (continue -> poÄ�etak petlje)
